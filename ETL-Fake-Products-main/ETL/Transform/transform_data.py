...
def main():
    ...
    try:
        ...
        remove_expression = r'[;/*-+:]+'
        df_transformed['description'] = df_transformed['description'].str.replace(remove_expression, '', regex=True)

        #Adicionando coluna 'preco_promocional'
        df_transformed['preco_promocional'] = (df_transformed['price'] * 0.09).round(2)

        #Retirando variaveis do dicionario e estruturando no json
        df_transformed['classificacao'] = df_transformed['rating'].apply(lambda x: x["rate"])
        df_transformed['contagem_de_classificacao'] = df_transformed['rating'].apply(lambda x: x["count"])
        df_transformed = df_transformed.drop(columns=['rating'])

        #Salvando alterações no JSON
